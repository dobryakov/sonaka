...
- Q: Как учитывать частично выполненные/частично оплаченные заказы? → A: Учитывать только полностью завершённые/оплаченные; частичные исключать
- Q: Что включать в «Общую сумму заказанного» на дашборде? → A: Только сумму товаров; налоги и доставка не включаются
- Q: Как считать «Общую сумму» при мультивалюте? → A: Конвертировать по историческому курсу заказа (в валюту дашборда)
- Q: Как учитывать частичные возвраты/рефанды после завершения заказа? → A: Исключать заказ целиком при частичном возврате
- Q: Пагинация списка заказов: размеры страницы и пределы? → A: Бесконечная лента (ленивая подгрузка)
...
- Очень большое число заказов — бесконечная лента (ленивая подгрузка) списка.
...
- Частично выполненные/частично оплаченные — исключаются из списков и расчётов сумм.
- Частичные возвраты/рефанды — заказ исключается целиком из списков и сумм.
...
- **FR-002**: Система должна отображать «Общую сумму заказанного» как сумму стоимостей товарных позиций по релевантным заказам (без налогов и доставки), согласно правилам включения статусов [см. FR-010]. При наличии разных валют суммы конвертируются в валюту дашборда по историческому курсу заказа.
...
- **FR-009**: Список заказов должен поддерживать масштабируемость через бесконечную ленту (ленивая загрузка) для больших историй.
- **FR-010**: В «список заказов» и «общую сумму заказанного» включаются только полностью завершённые/оплаченные заказы; частично выполненные/частично оплаченные, отменённые и возвращённые исключаются.
...
- **FR-013**: Если по заказу был произведён частичный возврат/рефанд, такой заказ исключается целиком из «списка заказов», «общей суммы заказанного» и списка «ранее купленных».
